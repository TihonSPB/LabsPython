...
class User(UserMixin, db.Model): # Класс наследуется от db.Model для работы с БД и миксин-класса для управлениями сессиями пользователей
    ...
    def follow(self, user):
        ...
        Метод для подписки текущего пользователя на другого пользователя.
        """
        if not self.is_following(user): # Проверяем, есть ли уже подписка
            self.following.add(user) # Добавляем запись в таблицу followers

    def unfollow(self, user):
        """
        ОТПИСКА ОТ ПОЛЬЗОВАТЕЛЯ
        Метод для отписки текущего пользователя от другого пользователя.
        """
        if self.is_following(user): # Проверяем, есть ли подписка
            self.following.remove(user) # Удаляем запись из таблицы followers

    def is_following(self, user): 
        """
        ПРОВЕРКА ПОДПИСКИ
        Метод проверяет, подписан ли текущий пользователь на указанного пользователя.
        Аргументы:
            user (User): Объект пользователя для проверки подписки        
        Возвращает:
            bool: True - если текущий пользователь подписан на user
                  False - если не подписан
        """
        # self.following.select() - создает базовый запрос SELECT для таблицы подписок
        # .where(User.id == user.id) - фильтрует записи по id проверяемого пользователя
        query = self.following.select().where(User.id == user.id)
        # db.session.scalar(query) - выполняет запрос и возвращает первое значение
        # is not None - преобразует результат в булево значение
        return db.session.scalar(query) is not None

    def followers_count(self):
        """
        КОЛИЧЕСТВО ПОДПИСЧИКОВ
        Подсчитывает, сколько пользователей подписано на текущего пользователя.
        Возвращает:
            int: Количество подписчиков (людей, которые подписаны на текущего пользователя)
        """
        # sa.select(sa.func.count()) - создает запрос с функцией COUNT()
        # .select_from() - указывает, что COUNT нужно выполнить над подзапросом
        # self.followers.select() - создает SELECT запрос для получения всех подписчиков
        # .subquery() - превращает этот запрос в подзапрос для использования в FROM
        query = sa.select(sa.func.count()).select_from(
            self.followers.select().subquery())
        # db.session.scalar(query) - выполняет запрос и возвращает число
        return db.session.scalar(query)

    def following_count(self):
        """
        КОЛИЧЕСТВО ПОДПИСОК
        Подсчитывает, на скольких пользователей подписан текущий пользователь.
        Возвращает:
            int: Количество подписок (людей, на которых подписан текущий пользователь)
        """
        # sa.select(sa.func.count()) - создает запрос с COUNT()
        # .select_from() - применяет COUNT к подзапросу
        # self.following.select() - создает SELECT запрос для получения всех подписок
        # .subquery() - превращает этот запрос в подзапрос
        query = sa.select(sa.func.count()).select_from(
    ...
    def following_posts(self):
        ...
        Формирует запрос для получения постов от пользователей, на которых подписан
        текущий пользователь, ВКЛЮЧАЯ свои собственные посты.
        Возвращает:
            SQLAlchemy Select объект: Запрос для получения персонализированной ленты
        Как работает запрос:
            1. Выбирает все посты из таблицы Post
            2. Присоединяет авторов этих постов
            3. Присоединяет подписчиков авторов
            4. Фильтрует: показывать только если текущий пользователь есть в подписчиках автора или сам является автором
            5. Группирует для устранения дубликатов
            6. Сортирует по времени публикации
        """
        # Псевдонимы для одной и той же модели User в разных ролях
        Author = so.aliased(User)   # Пользователь как автор поста
        Follower = so.aliased(User) # Пользователь как подписчик
        return (
            # 1. ВЫБИРАЕМ ВСЕ ПОЛЯ ИЗ МОДЕЛИ POST
            sa.select(Post)
            # 2. ПРИСОЕДИНЯЕМ АВТОРА К ПОСТУ
            # .join(Post.author) - стандартный JOIN через relationship 'author' в модели Post
            # .of_type(Author) - указываем, что автор должен рассматриваться как "Author" (псевдоним)
            # Результат: Каждый пост теперь имеет информацию о своем авторе (но под именем Author)
            .join(Post.author.of_type(Author))
            # 3. ПРИСОЕДИНЯЕМ ПОДПИСЧИКОВ АВТОРА (LEFT OUTER JOIN!)
            # Author.followers - relationship из модели User (те, кто подписан на автора)
            # .of_type(Follower) - указываем, что подписчики рассматриваются как "Follower"
            # isouter=True означает LEFT OUTER JOIN - сохраняем все посты, даже если у автора нет подписчиков
            .join(Author.followers.of_type(Follower), isouter=True)
            # 4. ФИЛЬТРАЦИЯ: два условия, объединенных через OR
            .where(sa.or_(
                Follower.id == self.id, # Условие 1: текущий пользователь подписан на автора
                Author.id == self.id, # Условие 2: текущий пользователь сам является автором
            ))
            # 5. ГРУППИРОВКА: убираем дубликаты постов
            # Без GROUP BY один пост может появиться несколько раз, если:
            # - Автор имеет несколько подписчиков
            # - Пост удовлетворяет обоим условиям WHERE
            .group_by(Post)
            # 6. СОРТИРОВКА: По времени публикации (новые сверху)
